...
Your SOLE purpose is to answer the user's request by following a strict, two-phase process.
...
1.  **OBEY IMMEDIATE TASK**: If a 'nextTasks' command exists in '🎯 YOUR IMMEDIATE TASK', execute it immediately. This is your only priority.

2.  **IF NO COMMAND, DECIDE**:
...
${toolDefinitions}`;
  ...
  // 4. STATE & HISTORY: The model's "memory", most recent first.
...
${reportEntries}`;
  ...
  private buildConversationHistorySection(conversationEntries: ConversationEntry[], limitNote: string): string {
    ...
    const formattedEntries = conversationEntries.map(entry => {
      let content = '';
      if (entry.user) content += `\n> User: "${entry.user}"`;
      if (entry.ai) content += `\n> Agent: (Responded with tool calls)`;
      return content;
    }).join('');

    return `# 💬 CONVERSATION HISTORY (FOR CONTEXT ONLY)
**WARNING**: This is outdated information. Use it ONLY to understand the user's goal, not for data.
${limitNote}
${formattedEntries}`;
  }

  // 5. ERROR RECOVERY: Specific instructions for when things go wrong.
...
- **If you need to just report status**: Use ${finalToolName} + ${reportToolName} instead of ${reportToolName} alone.`;
  ...
  // 6. THE TASK: The final, immediate command.
...
- **NEVER** use ${reportToolName} by itself`;
  ...
  buildPrompt(params: BuildPromptParams): string {
    ...
    // 4. State & History: Your memory of what has been done.
    ...
    // 5. Error Handling (if applicable): A special state that overrides the normal flow.
    if (lastError) {
    ...
    // 7. The Final Command: The specific, immediate action to take.
